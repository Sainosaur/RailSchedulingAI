...
class ValidationLayer:
    ...
    def get_segment(self, x: float) -> VLSegment:
        if x <= self.segments[0].start: return self.segments[0]
        if x >= self.segments[-1].end: return self.segments[-1]
        for seg in self.segments:
            if seg.start <= x < seg.end: return seg
        return self.segments[-1]

    def compute_dtz(self, x: float) -> float:
        seg = self.get_segment(x)
        for boundary in seg.block_boundaries:
            if boundary > x: return boundary - x
        return seg.spatial_headway

    # --- In validator.py _speed_for_aspect ---

def _speed_for_aspect(self, aspect: int, segment: VLSegment, dtz: float, current_v: float) -> Tuple[float, float]:
    ...
    def _project(self, x: float, u: float, proposed_a: float, v_ceil: float, t: float) -> Tuple[float, float]:
        if proposed_a == 0: return x + u * t, u
        if proposed_a > 0:
    ...
    def _check_action_safety(self, proposed_a: float, env_aspect: int, x: float, u: float, dtz: float) -> Tuple[bool, str]:
        ...
        for t in sim_steps:
            ...
            if v > max_safe_v + 0.1: return False, "Kinematic_Target_Violation"
            if v > v_ceiling + 0.1: return False, f"{current_seg.id}_Limit"
        return True, ""
