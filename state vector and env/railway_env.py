...
class RailwayEnv(gym.Env):
    def __init__(self):
        ...
        self.action_space = gym.spaces.Discrete(4) # generate 0/1/2/3 with particular probability(should be adjusted later)
        self.reset()
    ...
    def _get_obs(self):
        headway = abs(self.train1_pos - self.train2_pos) * 10 # simple headway calculation
        return np.array([self.train1_pos, self.train2_pos, self.train1_speed, self.train2_speed,
                         headway, self.train1_delay, self.train2_delay], dtype = np.float32)
    def step(self, action): # determine action for values correspond to action space
        ...
        if(self.train1_pos == 5 and self.train2_pos == 0): # stop if two trains reach terminus
            terminated = True
